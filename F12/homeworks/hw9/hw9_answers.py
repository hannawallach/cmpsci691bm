...
class LDA(object):
    ...
    def optimize_alpha_m(self, num_itns=5):
        ...
        for itn in xrange(1, num_itns + 1):
            ...
            new_alpha_m *= (psi(Ntd + tile(new_alpha_m, (D, 1))) - psi(tile(new_alpha_m, (D, 1)))).sum(axis=0) / (psi(Nd + new_alpha) - psi(new_alpha)).sum()
    ...
    def optimize_beta(self, num_itns=5):
        ...
        for itn in xrange(1, num_itns + 1):
            ...
            new_beta *= (psi(Nvt + tile(new_beta_n, (T, 1)).T) - psi(tile(new_beta_n, (T, 1)).T)).sum() / (V * (psi(Nt + new_beta) - psi(new_beta)).sum())
