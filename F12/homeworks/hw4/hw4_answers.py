...
def log_evidence_1(corpus, alpha, m, beta, n):
    ...
    log_e = gammaln(alpha) - gammaln(alpha * array(m)).sum() + gammaln(Dt + alpha * array(m)).sum() - gammaln(D + alpha)

    for t in xrange(T):
        log_e += gammaln(beta) - gammaln(beta * array(n)).sum() + gammaln(Nvt[:, t] + beta * array(n)).sum() - gammaln(Nt[t] + beta)
...
def log_evidence_2(corpus, alpha, m, beta, n):
    ...
    for d, (doc, t) in enumerate(corpus):
        log_e += log((Dt[t] + alpha * m[t]) / (d + alpha))
        Dt[t] += 1
        for v in doc.w:
            log_e += log((Nvt[v, t] + beta * n[v]) / (Nt[t] + beta))
            Nvt[v, t] += 1
            Nt[t] += 1
...
def posterior_mean_phis(corpus, beta, n):
    ...
    mean_phis = zeros((V, T), dtype=float)

    for t in xrange(T):
        mean_phis[:, t] = (Nvt[:, t] + beta * array(n)) / (Nt[t] + beta)
...
def posterior_mean_theta(corpus, alpha, m):
    ...
    mean_theta = (Dt + alpha * array(m)) / (D + alpha)
