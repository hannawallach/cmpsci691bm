...
def preprocess(filename, stopword_filename=None):
    ...
    return vocab, [vocab[x] for x in tokenize(text, stopwords)]
...
def evidence_1(corpus, beta, mean):
    ...
    return gamma(beta) / prod(gamma(beta * array(mean))) * prod(gamma(Nv + beta * array(mean))) / gamma(N + beta)
...
def evidence_2(corpus, beta, mean):
    ...
    for n in xrange(N):
        e *= (Nv[corpus[n]] + beta * mean[corpus[n]]) / (n + beta)
        Nv[corpus[n]] += 1
...
def log_evidence_1(corpus, beta, mean):
    ...
    return gammaln(beta) - gammaln(beta * array(mean)).sum() + gammaln(Nv + beta * array(mean)).sum() - gammaln(N + beta)
...
def log_evidence_2(corpus, beta, mean):
    ...
    for n in xrange(N):
        log_e += log((Nv[corpus[n]] + beta * mean[corpus[n]]) / (n + beta))
        Nv[corpus[n]] += 1
...
def posterior_mean(corpus, beta, mean):
    ...
    return (Nv + beta * array(mean)) / (N + beta)
...
def log_predictive_prob(new_corpus, corpus, beta, mean):
    ...
    new_beta = len(corpus) + beta
    new_mean = posterior_mean(corpus, beta, mean)

    return log_evidence_1(new_corpus, new_beta, new_mean)
